...
const getShop = async (req, res) => {
    ...
    if (req.query.minPrice && req.query.maxPrice) {
        if (searchQuery) {
            priceRangeQuery = " AND Price BETWEEN ? AND ?";
        } else {
            priceRangeQuery = " WHERE Price BETWEEN ? AND ?";
        }
        searchParams.push(parseFloat(req.query.minPrice), parseFloat(req.query.maxPrice));
    }
  
    try {
        let query = `SELECT * FROM product ${searchQuery} ${priceRangeQuery} ${priceOrder} ${idOrder} LIMIT ?, ?`;
        if ((searchQuery || priceRangeQuery) && totalProduct[0].total === 1) {
            query = `SELECT * FROM product ${searchQuery} ${priceRangeQuery}`;
